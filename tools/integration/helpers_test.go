...
import (
	...
	"extension-scaffold/tools/pkg/contracts/cleat"
...
func deployFreshInstructionSender(t *testing.T) (common.Address, *cleat.CleatInstructionSender) {
	t.Helper()
	time.Sleep(rpcDelay)
	...
	address, tx, contract, err := cleat.DeployCleatInstructionSender(
...
func deployInstructionSenderRaw(t *testing.T, registryAddr, machineRegistryAddr common.Address) error {
	...
	_, _, _, err = cleat.DeployCleatInstructionSender(
		opts, testSupport.ChainClient,
